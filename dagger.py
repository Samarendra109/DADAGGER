...
def get_cumulative_cross_track_error(cross_track_error_list):
    max_length_travelled = max(len(cross_track_errors) for cross_track_errors in cross_track_error_list)
    ...
    for cross_track_errors in cross_track_error_list:
        ...
        cumulative_loss = sum(abs(e) for e in cross_track_errors) + \
                          abs(cross_track_errors[-1]) * (max_length_travelled - len(cross_track_errors))
...
if __name__ == "__main__":
    ...
    args.weights_out_file = "./weights/learner_%d.weights" % 0
    #print ('TRAINING LEARNER ON INITIAL DATASET')
    learner_policies = [train_policy.main(args)]
    cross_track_error = []

    for i in range(args.dagger_iterations):
        args.weights_out_file = "./weights/learner_%d.weights" % (i + 1)
        args.run_id = 100+i #Adding 100 to distinguish between existing dataset and expert dataset
    ...
    for i in range(args.dagger_iterations+1):
        args.weights_out_file = "./weights/learner_%d.weights" % i
    ...
    with open("./weights/cross_track_error_list.errors", "wb") as f:
        pickle.dump(cross_track_error_list, f)

    cumulative_cross_track_errors = get_cumulative_cross_track_error(cross_track_error_list)

    with open("./weights/cum_cross_track_error.errors", "wb") as f:
    ...
    fig.savefig('./dataset_iterations.png')
    plt.close(fig)
