...
def train_discrete(model, iterator, opt, args):
    ...
    for i_batch, batch in enumerate(iterator):
        ...
        weights = 1/(torch.from_numpy(args.class_dist) + 1) if args.weighted_loss else torch.ones_like(
            torch.from_numpy(args.class_dist))
...
def main(args):
    ...
    training_iterator = DataLoader(training_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2,
                                   sampler=WeightedRandomSampler(get_sample_weights(training_dataset),
